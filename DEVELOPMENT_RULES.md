...
- 흡입 판정 시점은 두 곳뿐이다: **셀 중앙에서 커밋하는 순간**(`_finish_step()`)과 **손을 떼는 순간**(`set_grabbed(false)`, 정지 상태일 때). 전이 중에 시작하면 레일과 점유가 어긋난다.
- **드래그 중에는 흡입하지 않는다.** 잡은 채 짝 구멍 옆칸을 지나가거나 옆칸에 세워 둬도 걸리지 않고, 손을 떼야 들어간다. `DragController` 가 잡을 때 `set_grabbed(true)`, 놓을 때 `false` 를 넣는다.
...
- **판정 대상은 리드(잡았던 끝, `body_cells[0]`) 하나뿐**이며 4방향 인접만 본다. 대각선은 걸리지 않는다. 머리 이동 중 꼬리가 구멍을 스쳐도 들어가지 않는다 — 반대쪽 끝으로 넣으려면 그 끝을 잡아 리드로 만든 뒤 손을 떼야 한다(제자리에서 잡았다 놓기만 해도 된다).
