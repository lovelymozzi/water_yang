...
# 생성기의 소프트 게이트와 이 커브가 다른 기준으로 재면 "점수는 통과했는데 커브에서는 쉬워
# 보이는" 판이 생긴다. 점수가 없는 옛 파일만 저장된 탈출 비용에서 다시 잰다.
static func difficulty_of(level: Dictionary) -> float:
	return float(StageBatch.score_of(level))
...
2중첩은 색·구멍 1개, 3중첩은 2개가 더 필요하다. 팔레트 색 수가 모자라면 생성하지 않는다.")
	...
	_add_spin(params_grid, "pack", "몸이 채우는 비율", 0.2, 0.8, 0.05, 0.7,
	...
	_add_spin(params_grid, "dep_slack", "독립 고양이 허용", 1, 8, 1, 8,
	...
	_add_spin(params_grid, "score", "난이도 점수 하한", 0, 400, 5, 0,
		"실측 난이도 점수(탈출별 드래그 비용의 감쇠 가중합 ×10)가 이 값 미만이면 버린다.\n0 = 검사 안 함. 조건을 하나씩 하드하게 강제하는 대신 이 한 값만 본다.\n참고: 기존 55스테이지 88, 가장 어려웠던 36스테이지 121.")
...
func _on_batch_generate() -> void:
	...
	for index in count:
		...
		# 사슬 깊이는 요구하지 않는다. "나머지가 전원 시작 자리에 얼어 있다"는 가정에서 재는
		# 값이라 실제 난이도와 상관이 없고(실측 상관 0.24), 요구하면 버리는 맵만 늘어난다.
		# 난이도는 "난이도 점수 하한"(소프트 게이트) + 생성 후 점수 정렬이 책임진다.
		...
		# 난이도 순 배당은 생성 때 하지 않는다 — 게임이 완성된 뒤 전체 맵을 놓고 한 번에
		# 하는 별도 작업이다. 그 재료가 `stats.difficulty_score`(난이도 곡선이 읽는 값)다.
